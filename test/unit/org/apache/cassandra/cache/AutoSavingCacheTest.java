...
public class AutoSavingCacheTest extends SchemaLoader
{
    ...
    public void testSerializeAndLoadKeyCache() throws Exception
    {
        ...
        for (int i = 0; i < 2; i++)
        {
            RowMutation rm = new RowMutation("Keyspace1", ByteBufferUtil.bytes("key1"));
            rm.add("Standard1", ByteBufferUtil.bytes("c1"), ByteBufferUtil.bytes(i), 0);
            rm.apply();
        ...
        for (SSTableReader sstable : cfs.getSSTables())
            assert keyCache.get(new KeyCacheKey(sstable.descriptor, ByteBufferUtil.bytes("key1"))) != null;
    }
}
