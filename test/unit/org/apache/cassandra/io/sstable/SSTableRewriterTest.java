...
public class SSTableRewriterTest extends SSTableWriterTestBase
{
    ...
    public void testSSTableSectionsForRanges() throws IOException, InterruptedException, ExecutionException
    {
        ...
        Collections.singleton(cfs), 0L, false, PreviewKind.NONE);
        ...
        Runnable r = new Runnable()
        {
            public void run()
            {
                while (!done.get())
                {
                    ...
                    List<StreamSession.SSTableStreamingSections> sections = StreamSession.getSSTableSectionsForRanges(range, Collections.singleton(cfs), 0L, false, PreviewKind.NONE);
                    if (sections.size() != 1)
                        failed.set(true);
