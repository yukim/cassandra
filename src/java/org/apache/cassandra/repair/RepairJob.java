...
public class RepairJob
{
    ...
    public RepairJob(UUID sessionId, String keyspace, String columnFamily, Range<Token> range, boolean isSequential)
    {
        ...
        this.treeRequests = new RequestCoordinator<InetAddress>(isSequential)
        {
            ...
            {
                ValidationRequest request = new ValidationRequest(gcBefore);
                RepairMessage<ValidationRequest> message = new RepairMessage<>(new RepairMessageHeader(desc, RepairMessageType.VALIDATION_REQUEST), request);
    ...
    public void sendTreeRequests(Collection<InetAddress> endpoints)
    {
        ...
        this.gcBefore = (int)(System.currentTimeMillis() / 1000) - Table.open(desc.keyspace).getColumnFamilyStore(desc.columnFamily).metadata.getGcGraceSeconds();
