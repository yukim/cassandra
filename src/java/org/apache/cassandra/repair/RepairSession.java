...
public class RepairSession extends WrappedRunnable implements IEndpointStateChangeSubscriber, IFailureDetectionEventListener
{
    ...
    public void validationComplete(RepairJobDesc desc, InetAddress endpoint, MerkleTree tree)
    {
        ...
        if (tree == null)
            logger.info(String.format("[repair #%s] Validation failed for %s from %s", getId(), desc.columnFamily, endpoint));
        else
    ...
    {
        logger.debug(String.format("[repair #%s] Repair " + (success ? "completed" : "failed") + " between %s and %s on %s",
                                          getId(),
                                          nodes.endpoint1,
                                          nodes.endpoint2,
                                          desc.columnFamily));
        ...
        {
            RepairJob completedJob = activeJobs.remove(job.desc.columnFamily);
            ...
            {
                logger.info(String.format("[repair #%s] %s sync failed%s", getId(), completedJob.desc.columnFamily, remaining));
            }
            else
            {
