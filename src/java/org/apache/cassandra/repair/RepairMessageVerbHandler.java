    ...
    public void doVerb(MessageIn<RepairMessage<?>> message, int id)
    {
        ...
        switch (header.type)
        {
            case VALIDATION_REQUEST:
                ValidationRequest validationRequest = (ValidationRequest) message.payload.body;
                // trigger read-only compaction
                ColumnFamilyStore store = Table.open(header.desc.keyspace).getColumnFamilyStore(header.desc.columnFamily);
                Validator validator = new Validator(header.desc, message.from, validationRequest.gcBefore);
