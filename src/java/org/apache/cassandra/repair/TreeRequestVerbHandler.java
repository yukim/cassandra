...
/**
 * Handler for requests from remote, older version nodes to generate a valid tree.
 */
@Deprecated
public class TreeRequestVerbHandler implements IVerbHandler<TreeRequest>
{
    ...
    {
        TreeRequest request = message.payload;

        // trigger read-only compaction
        ColumnFamilyStore store = Table.open(request.desc.keyspace).getColumnFamilyStore(request.desc.columnFamily);
        Validator validator = new Validator(request.desc, message.from, request.gcBefore);
