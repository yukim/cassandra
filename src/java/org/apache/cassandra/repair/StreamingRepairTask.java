...
public class StreamingRepairTask implements Runnable, IStreamCallback
{
    ...
    public void onSuccess()
    {
        ...
        {
            // we have to respond with older format for node whose version is older than 2.0
            MessageOut<UUID> message = new MessageOut<>(MessagingService.Verb.STREAMING_REPAIR_RESPONSE, desc.sessionId, UUIDSerializer.serializer);
            MessagingService.instance().sendOneWay(message, request.initiator);
        }
        else
        {
