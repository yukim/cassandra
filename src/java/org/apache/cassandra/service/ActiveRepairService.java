    ...
    public void handleMessage(InetAddress endpoint, RepairMessage message)
    {
        ...
        switch (message.header.type)
        {
            case VALIDATION_COMPLETE:
                ...
                session.validationComplete(desc, endpoint, tree);
                break;
            case VALIDATION_FAILED:
                session.validationComplete(desc, endpoint, null);
                break;
            case SYNC_COMPLETE:
                // one of replica is synced.
                session.syncComplete(desc, (NodePair) message.body, true);
                break;
            case SYNC_FAILED:
                // one of streaming failed
                session.syncComplete(desc, (NodePair) message.body, false);
                break;
            default:
                break;
        }
